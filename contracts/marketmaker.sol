...
contract MarketMaker {
    ...
    function getCurrentPrice(uint256 _marketId, uint256 _outcomeIndex, uint256 _amount) public view returns (uint256) {
        ...
        if (_outcomeIndex == 0) {
            return (market.constantProduct / (market.yesCount + _amount)) - market.noCount;
        } else {
            return (market.constantProduct / (market.noCount + _amount)) - market.yesCount;
        }
    }

    function getProbability(uint256 _marketId, uint256 _outcomeIndex) public view returns (uint256) {
        Market storage market = markets[_marketId];
        require(_outcomeIndex < 2, "Invalid outcome index");

        if (_outcomeIndex == 0) {
            return (market.yesCount * 1e18) / (market.yesCount + market.noCount);
        } else {
            return (market.noCount * 1e18) / (market.yesCount + market.noCount);
        }
    }
